...
exports.uploadStudentImage = (req, res) => {
  ...
  uploadimage(req, res, function (err) {
    ...
    const { userrole, uuid } = req; // Destructure uuid from req

    // Log the student ID and role from the request parameters
    ...
    if (userrole === 'student') {
      ...
      Student.findByPk(studentId)
        .then((student) => {
          ...
          // Update student's image and identity image
          student.update({
            photo: req.files['image'][0].path,
            identityImage: req.files['identityImage'][0].path
          })
      ...
      // Handle admin or user-specific logic using uuid
      User.findByPk(uuid)
        .then((user) => {
          if (!user) {
          ...
          // Update user's image and identity image
          user.update({
            photo: req.files['image'][0].path,
            identityImage: req.files['identityImage'][0].path
          })
            ...
            .catch((error) => {
              ...
              res.status(500).json({ message: "Failed to update user images", error });
            });
        })
        .catch((error) => {
